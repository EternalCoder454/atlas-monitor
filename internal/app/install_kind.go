...
func updateCommand(manager, pkg, aurHelper string) string {
	...
	case "pacman":
		...
		// for, and `pacman -Syu atlas-monitor` would answer "target not found".
		// Rebuilding from the newer PKGBUILD is what actually works.
		return "git clone " + repoURL + ".git && cd atlas-monitor/packaging && makepkg -si"
	case "apt":
